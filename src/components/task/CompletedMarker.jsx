 ...
 * color (§5):
 *  - confirmed (manager accepted) -> white check on a filled green disc — a "closed" win.
 *  - completed (awaiting acceptance) -> bare green check, no circle — done, ring not closed.
 *
 * Render before the title when `task.status` is 'completed' or 'confirmed' (or `task.completed`).
 * The matching StatusPill ("Laukia priėmimo"/"Priimtas") still carries the word.
 *
 * @param {Object} props
