...
describe('notification copy lockstep (functions copyForRequestNotification ↔ client registry)', () => {
  ...
  const SAMPLES = {
    ...
    // Worker → admin FYI that a session's START was self-corrected (either direction). Same shape.
    time_self_start_corrected: [{ userName: '  Simona   Petraitienė ', day: '2026-07-28' }, { day: '2026-07-28' }, {}],
...
describe('canonical ledger-id lockstep (client engine closers ↔ functions nets)', () => {
  ...
  it('both nets retire the canonical record for the run they close', () => {
    const autoStop = FUNCTIONS_SRC.slice(
      ...
      FUNCTIONS_SRC.indexOf('const TIMER_OVER_ESTIMATE_QUIET_MS')
    );
