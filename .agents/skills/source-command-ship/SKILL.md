...
`/ship` is itself the human-initiated trigger AGENTS.md requires: deploy is never autonomous,
...
- **Worktree-safe.** WORKZ work happens in worktrees under `.Codex/worktrees/`; `main` is
...
- **Gate before prod.** `npm run lint` (zero warnings) + `npm run build` must pass before the
  push — per AGENTS.md's quality gate. A broken build must never reach `main`.
...
- Otherwise compose a commit. Read the staged diff (`git diff --cached --stat` and the
  ...
  carry the WORKZ metadata block (AGENTS.md). `Reason:` is English, persisted.
...
[ai-author: Codex-opus-4-8]
Reason: <one line — why this change is correct / what rule or goal it serves>
EOF
```

> Use the running model's id in `[ai-author: ...]` (e.g. `Codex-opus-4-8`).
...
> spuriously. Lint + build is the gate the user chose and AGENTS.md mandates.
...
- **No Firestore/Storage rules deploy.** `firestore.rules` / `storage.rules` ship via a
  ...
  AGENTS.md) — a code push to main does not deploy rules.
