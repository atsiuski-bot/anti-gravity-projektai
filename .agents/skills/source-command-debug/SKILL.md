...
code.** Everything written during this session (committed on the `Codex/*` branch, staged,
...
Severity scale:
- 🔴 **Critical** — breaks prod / crosses the human-only boundary / DANGER zone (AGENTS.md)
...
- `git rev-parse --show-toplevel` + `git branch --show-current` — confirm we are in a
  `Codex/*` worktree. If `main` → **STOP** "not a worktree — nothing to diagnose" (the only
...
1. **Raw `err.message` rendered to the user** — pattern `err\.message|error\.message|e\.message`
   ...
   JSX text → 🔴 "AGENTS.md bans raw `err.message` in UI — map to friendly Lithuanian copy".
...
3. **Retired brand** — pattern `Viduramžiai|Viduramzi|viduramziai` glob `src/**/*.{js,jsx}`
   ...
   🟠 "WORKZ is the only name — the old brand is retired everywhere (AGENTS.md)".
4. **Hardcoded session color** — pattern `bg-(red|blue|amber|green|orange)-[0-9]` or raw hex
   ...
   (AGENTS.md / DESIGN_SYSTEM §Calm canvas, loud state); a stray inline color breaks the
...
6. **English user-facing copy** — scan touched `src/**/*.jsx` for newly-added button labels /
   ...
   (AGENTS.md language policy); English belongs only in code/comments/commits". (This is the
